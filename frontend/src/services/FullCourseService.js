...
export const fetchFullCourseDataFromAPI = async (courseId, token) => {
  ...
  const response = await fetch(`http://localhost:4000/api/course/full/${courseId}`, {
    method: 'GET',
    headers: {
...
export const enrollUserInCourse = async (courseId, token) => {
  console.log('Enrolling user in course');

  if (!token) {
    throw new Error('Unauthorized: Please log in to enroll in this course');
  }

  const response = await fetch(`http://localhost:4000/api/course/enroll/${courseId}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
  ...
  if (!response.ok) {
    if (response.status === 401) {
      ...
      throw new Error('Forbidden: You do not have permission to enroll in this course');
    } else {
...
export const updateCachedCourseData = (courseId, updatedData) => {
  const cachedData = localStorage.getItem(`${FULL_COURSE_KEY}_${courseId}`);
  if (!cachedData) return;

  const { timestamp, refreshCount } = JSON.parse(cachedData);
  const newCache = {
    data: updatedData,
    timestamp,
    refreshCount
  };
  localStorage.setItem(`${FULL_COURSE_KEY}_${courseId}`, JSON.stringify(newCache));
};
